...
def qwen_actions(env, obs, dialog_queue, dialog4ref_queue, qwen_path, qwen_error, total_cost):
    if not os.path.exists(qwen_path):
        os.makedirs(qwen_path)
    ...
    for idx in range(env.num_agents):
        ...
        obs_prompt = f'''
                        ...
                        2. NO explanations, NO thinking process, NO extra text.
                        3. Format: {{"work": 0.XX, "consumption": 0.XX}}.
    ...
    if env.world.timestep%3 == 0 and env.world.timestep > 0:
        ...
        dialogs_list = [list(dialogs)[:2] + list(dialog4ref)[-3:-1] + list(dialogs)[-1:] 
                       for dialogs, dialog4ref in zip(dialog_queue, dialog4ref_queue)]
        
        for batch_start in range(0, len(dialogs_list), batch_size):
            batch_end = min(batch_start + batch_size, len(dialogs_list))
            batch_dialogs = dialogs_list[batch_start:batch_end]
            
            print(f"📦 处理 batch {batch_start//batch_size + 1}: agents {batch_start}-{batch_end-1}")
            
            batch_results, batch_cost = get_multiple_completion(batch_dialogs)
        ...
        dialogs_list = [list(dialogs) for dialogs in dialog_queue]
    ...
    def _extract_json(text: str) -> str:
        ...
        # 4. 查找包含 work 和 consumption 的 JSON
        m = re.search(r'\{[^}]*"work"[^}]*"consumption"[^}]*\}', text)
        if m:
            return m.group(0)
    ...
    #     content = results[idx]
    #     try:
    #         content_clean = _extract_json(content)  # ← 关键：清理输出
    #         extracted_actions = list(eval(content_clean).values())
    #         if not action_check(extracted_actions):
    #             extracted_actions = [1, 0.5]
    #             qwen_error += 1
    #     except Exception:
    ...
    for idx in range(env.num_agents):
        ...
        # ============ 实际运行诊断 ============
        if idx == 0:  # 打印每轮的第一个 agent
            print(f"\n{'='*80}")
            print(f"🔍 [Timestep {env.world.timestep}] Agent 0 输出:")
            print(f"{'='*80}")
        ...
        try:
            ...
            if idx == 0:
                print(f"清理后: {content_clean[:100]}")
            
            extracted_actions = list(eval(content_clean).values())
            
            if idx == 0:
                print(f"提取的 actions: {extracted_actions}")
                print(f"action_check 结果: {action_check(extracted_actions)}")
            
            if not action_check(extracted_actions):
                if idx == 0:
                    print(f"❌ action_check 失败！")
                extracted_actions = [1, 0.5]
                qwen_error += 1
            else:
                if idx == 0:
                    print(f"✅ action_check 通过！")
        ...
        if idx == 0:
            print(f"最终 actions: {actions[str(idx)]}")
            print(f"{'='*80}\n")
    ...
    for idx, agent_dialog in enumerate(dialog_queue):
        with open(f'''{qwen_path}/{env.get_agent(str(idx)).endogenous['name']}''', 'a') as f:
    ...
    if (env.world.timestep+1)%3 == 0:
        ...
        for idx, agent_dialog in enumerate(dialog4ref_queue):
             with open(f'''{qwen_path}/{env.get_agent(str(idx)).endogenous['name']}''', 'a') as f:
                for dialog in list(agent_dialog)[-2:]:
                    f.write(f'''>>>>>>>>>{dialog['role']}: {dialog['content']}\n''')
    return actions, qwen_error, total_cost
...
def main(policy_model='qwen', num_agents=100, episode_length=240, dialog_len=3, beta=0.1, gamma=0.1, h=1, max_price_inflation=0.1, max_wage_inflation=0.05, resume_from=None):
    # ========== 首先检查是否 resume ==========
    start_epi = 0
    ...
    if resume_mode:
        ...
        match = re.search(r'_(\d+)\.pkl$', resume_from)
        if match:
            start_epi = int(match.group(1))
            print(f"✅ Will resume from episode {start_epi}")
    env_config['n_agents'] = num_agents
    env_config['episode_length'] = episode_length
    if policy_model in ['gpt', 'qwen']:
        total_cost = 0
    ...
    if resume_mode:
        # 加载 checkpoint
        print(f"📂 Loading environment state...")
        with open(resume_from.replace('dense_log', 'env'), 'rb') as f:
            env = pkl.load(f)
        with open(resume_from.replace('dense_log', 'obs'), 'rb') as f:
            obs = pkl.load(f)
        
        if policy_model in ['gpt', 'qwen']:
            print(f"💬 Loading dialog history...")
            with open(resume_from.replace('dense_log', 'dialog'), 'rb') as f:
                dialog_queue = pkl.load(f)
            with open(resume_from.replace('dense_log', 'dialog4ref'), 'rb') as f:
    ...
    for epi in range(start_epi, env.episode_length):
        if policy_model in ['gpt', 'qwen']:
            actions, qwen_error, total_cost = qwen_actions(env, obs, dialog_queue, dialog4ref_queue, f'{save_path}data/{policy_model_save}/dialogs', qwen_error, total_cost)
        elif policy_model == 'complex':
        ...
        if (epi+1) % 3 == 0:
            ...
            if policy_model in ['gpt', 'qwen']:
                print(f'#errors: {qwen_error}, cost ${total_cost:.1f} so far')
            t = time()
    ...
    if policy_model in ['gpt', 'qwen']:
        print(f'#gpt/qwen errors: {qwen_error}')
